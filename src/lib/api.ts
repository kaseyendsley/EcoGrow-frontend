...
export const API = {
  ...
  // UserQuest with file upload (reflection, rating, photo, completed_at)
  // NOW OPTIONAL: photo is optional; falls back to JSON when missing.
  completeUserQuestUpload: async (
    id: number,
    payload: { reflection: string; rating: number; photo?: File; completed_at?: string }
  ): Promise<UserQuest> => {
    if (payload.photo) {
    ...
    const { photo: _omit, ...jsonBody } = payload;
    return fetchJSON<UserQuest>(`/api/user-quests/${id}/complete/`, {
      method: "POST",
